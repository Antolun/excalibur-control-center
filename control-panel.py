...
class ColorSwatch(Static):
    ...
    ColorSwatch:hover { border: tall cyan; }
...
class ExcaliburApp(App):
    ...
    CSS = APP_CSS
    TITLE = "LupuS Excalibur WMI Control Center"
    SUB_TITLE = "v1.0"
    ...
    def _build_dashboard(self) -> Widget:
        return Vertical(
            ...
            Horizontal(
                ...
                FanGauge("𖣘  GPU Fan", id="gpu-gauge"),
                id="fan-section",
            ),
    ...
    def _build_about(self) -> Widget:
        ...
        driver_present = "Loaded Successfully ✓" if self._state.hwmon_path else "Not Detected ×"
        return Vertical(
            Static(ASCII_LOGO),
            Static(
...
def check_driver() -> None:
    ...
    if not led_glob and not hwmon:
        print(
            "\033[93m⚠  WARNING: excalibur-wmi-lupus driver does not appear to be loaded.\033[0m\n"
