...
class FanGauge(QFrame):
    ...
    def update_rpm(self, rpm):
        ...
        self.rpm_label.setStyleSheet(f"font-size: 20px; font-weight: bold; color: {color};")
        
        max_rpm = 6000
        filled = int((min(rpm, max_rpm) / max_rpm) * 20)
...
class ExcaliburControlPanel(QMainWindow):
    ...
    def apply_background(self):
        bg_path = os.path.join(os.path.dirname(__file__), "darkbg.jpg")
        if os.path.exists(bg_path):
            oImage = QImage(bg_path)
            sImage = oImage.scaled(self.size(), Qt.AspectRatioMode.IgnoreAspectRatio, Qt.TransformationMode.SmoothTransformation)
            palette = QPalette()
    ...
    def create_dashboard_tab(self):
        ...
        self.cpu_gauge = FanGauge("𖣘 CPU Fan")
        self.gpu_gauge = FanGauge("𖣘 GPU Fan")
